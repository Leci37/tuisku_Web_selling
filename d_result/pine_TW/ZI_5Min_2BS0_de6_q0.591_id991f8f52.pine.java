	...
	if( bbp > -0.081821 )
		if( bearPower <= 0.088098 )
			if( smoothK_k <= -1.17591 )
				ret := 0.157722
			if( smoothK_k > -1.17591 )
				if( smoothD_d <= 3.80418 )
					ret := -0.172185
				if( smoothD_d > 3.80418 )
					if( k <= 99.3987 )
						if( smoothD_d <= 80.8824 )
							ret := -0.016033
						if( smoothD_d > 80.8824 )
							ret := 0.057736
					if( k > 99.3987 )
...
float op_operation = decision_tree_0_ZI_5Min_991f8f52(bbp, bullPower, BBPower_Color, bearPower, bbm, smoothK_k, d_k, k, smoothD_d, rsi1, d)
