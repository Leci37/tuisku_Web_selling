			...
			if( ema1 <= 38.2906 )
				if( tema <= 19.6907 )
					...
					if( d_k > 2.54504 )
						ret := -0.666667
				if( tema > 19.6907 )
					if( smoothK_k <= -1.56837 )
						ret := -0.631579
					if( smoothK_k > -1.56837 )
						ret := 0.378788
			if( ema1 > 38.2906 )
				if( smoothK_k <= 91.32 )
					if( smoothD_d <= 18.9028 )
						ret := -0.069767
					if( smoothD_d > 18.9028 )
						ret := -0.383838
				if( smoothK_k > 91.32 )
...
float op_operation = decision_tree_0_NVST_1Day_922ac902(d, d_k, smoothK_k, k, rsi1, smoothD_d, ema12, ema3, ema13, tema, ema1, ema2)
