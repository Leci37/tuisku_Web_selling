			...
			if( dif_source_BBlower > 0.492603 )
				if( BBlower <= 148.879 )
					...
					if( BBlower > 146.375 )
						ret := -0.348837
				if( BBlower > 148.879 )
					if( dif_source_BBupper <= -3.4435 )
						ret := 0.476510
					if( dif_source_BBupper > -3.4435 )
						ret := 0.094905
		if( BBupper > 160.325 )
			if( dif_source_BBupper <= -22.2006 )
				if( BBlower <= 273.628 )
					if( vrsi <= 81.638 )
						if( BBlower <= 270.211 )
							ret := -0.088235
...
float op_operation = decision_tree_0_SPOT_5Min_f3e1e576(crossunder_rsi_bbupperInt, BBlower, dif_source_BBlower, dif_source_BBupper, vrsi, BBupper, crossover_rsi_bblowerInt)
