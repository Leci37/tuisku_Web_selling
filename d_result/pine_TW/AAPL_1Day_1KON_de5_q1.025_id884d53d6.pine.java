			...
			if( media > 25.4453 )
				if( pvi_ema <= 0.149963 )
					...
					if( pvimax > 0.115238 )
						ret := -0.888060 // sell
				if( pvi_ema > 0.149963 )
					if( media_azul <= 34.2862 )
						ret := -0.571429
					if( media_azul > 34.2862 )
						ret := 0.384615
		if( verde_media > -42.7944 )
			if( marron_mean <= 24.0374 )
				if( verde_azul <= -23.7254 )
					if( verde_mean <= -23.4952 )
						ret := -1.000000 // sell
					if( verde_mean > -23.4952 )
						ret := 0.000000
				if( verde_azul > -23.7254 )
...
float op_operation = decision_tree_0_AAPL_1Day_884d53d6(oscp, media, media_azul, verde_media, azul, tprice, azul_mean, verde, nvim, marron, pvimin, pvi, nvimax, verde_mean, xmf, source, stoc, pvimax, nvi_ema, nvi, xrsi, pvim, marron_mean, nvimin, BollOsc, verde_azul, pvi_ema)
