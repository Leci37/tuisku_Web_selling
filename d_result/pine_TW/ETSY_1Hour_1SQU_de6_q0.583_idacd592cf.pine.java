				...
				if( dev > 2.87253 )
					...
					if( ma > 162.986 )
						...
						if( upperBB > 175.087 )
							ret := 0.049025
		if( dev > 5.16802 )
			if( avgHighLow <= 216.291 )
				if( lowerKC <= 180.363 )
					if( avgHighLow <= 149.555 )
						if( upperBB <= 103.83 )
							ret := -0.396396
						if( upperBB > 103.83 )
							ret := -0.010945
...
float op_operation = decision_tree_0_ETSY_1Hour_acd592cf(sqzOnInt, priceRange, lowerKC, lowerBB, rangema, noSqzInt, basis, val, ma, dev, sqzOffInt, upperBB, upperKC, avgHighLow)
