			...
			if( upperKC <= 173.82 )
				...
				if( upperBB > 164.137 )
					...
					if( val > 1.45015 )
						ret := -0.500000
			if( upperKC > 173.82 )
				if( avgHighLow <= 172.353 )
					if( sqzOffInt <= 0.5 )
						ret := -0.909091 // sell
					if( sqzOffInt > 0.5 )
						ret := -0.454545
				if( avgHighLow > 172.353 )
					if( priceRange <= 12.4141 )
						ret := -0.481481
					if( priceRange > 12.4141 )
						ret := 0.333333
...
float op_operation = decision_tree_0_DDOG_1Day_3dd94f1d(sqzOnInt, priceRange, lowerKC, lowerBB, rangema, noSqzInt, basis, val, ma, dev, sqzOffInt, upperBB, upperKC, avgHighLow)
