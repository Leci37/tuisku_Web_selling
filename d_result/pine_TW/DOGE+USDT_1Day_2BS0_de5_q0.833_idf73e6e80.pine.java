			...
			if( d <= 76.3516 )
				...
				if( bearPower > 0.013729 )
					...
					if( d_k > 7.79004 )
						ret := -0.800000 // sell
			if( d > 76.3516 )
				if( bullPower <= 0.005453 )
					if( rsi1 <= 51.0364 )
						ret := 0.027397
					if( rsi1 > 51.0364 )
						ret := 0.626667
				if( bullPower > 0.005453 )
					if( smoothK_k <= 68.1349 )
						ret := 0.666667
					if( smoothK_k > 68.1349 )
						ret := -0.500000
		if( rsi1 > 78.2998 )
			if( smoothD_d <= 62.0879 )
				ret := 1.000000 // buy
			if( smoothD_d > 62.0879 )
				if( bbm <= 0.000798 )
					if( d_k <= 0.488424 )
...
float op_operation = decision_tree_0_DOGEUSDT_1Day_f73e6e80(bearPower, bbm, bbp, bullPower, BBPower_Color, d, d_k, smoothK_k, k, rsi1, smoothD_d)
