	...
	if( Positive_Money_Flow_Sum > 4.60911e+09 )
		if( Money_Flow_Ratio <= 0.435249 )
			...
			if( bbm > 11.9255 )
				ret := 1.000000 // buy
		if( Money_Flow_Ratio > 0.435249 )
			if( Raw_Money_Flow <= 2.93816e+09 )
				if( bullPower <= 5.84084 )
					if( bbp <= 1.4459 )
						ret := 0.068097
					if( bbp > 1.4459 )
						ret := -0.229070
				if( bullPower > 5.84084 )
					if( Typical_Price <= 93.425 )
						ret := -0.366667
					if( Typical_Price > 93.425 )
						ret := 0.318750
			if( Raw_Money_Flow > 2.93816e+09 )
...
float op_operation = decision_tree_0_PYPL_1Day_62c074f4(bearPower, bbm, bbp, bullPower, BBPower_Color, Positive_Money_Flow_Sum, Raw_Money_Flow, MFI_Low, Negative_Money_Flow, Negative_Money_Flow_Sum, Positive_Money_Flow, Money_Flow_Ratio, MFI, MFI_High, Typical_Price)
