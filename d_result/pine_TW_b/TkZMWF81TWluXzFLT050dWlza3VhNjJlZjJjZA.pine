				...
				if( oscp > -19.6584 )
					if( BollOsc <= -34.3831 )
						...
						if( nvim > -0.012774 )
							ret := 0.133333
					if( BollOsc > -34.3831 )
						if( verde_media <= 30.9959 )
							ret := -0.335347
						if( verde_media > 30.9959 )
							ret := 0.147887
			if( media > 79.0976 )
				if( pvi_ema <= 0.002205 )
					if( azul_mean <= 16.9774 )
						ret := -0.050847
					if( azul_mean > 16.9774 )
						ret := -0.821429 // sell
				if( pvi_ema > 0.002205 )
					if( marron_mean <= 83.9617 )
						ret := 0.062500
					if( marron_mean > 83.9617 )
						ret := -0.898551 // sell
...
float op_operation = decision_tree_0_NFLX_5Min_a62ef2cd(verde_mean, verde, marron, media_azul, verde_media, nvi_ema, stoc, azul, verde_azul, pvi_ema, tprice, pvi, marron_mean, pvimax, azul_mean, xrsi, nvimin, media, source, oscp, nvim, xmf, pvimin, BollOsc, nvi, pvim, nvimax)
