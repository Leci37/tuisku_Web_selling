			...
			if( MFI_High > -37.2395 )
				if( MFI <= 46.7008 )
					...
					if( Typical_Price > 144.066 )
						ret := -0.641509
				if( MFI > 46.7008 )
					if( Negative_Money_Flow <= 1.55711e+08 )
						ret := -0.147636
					if( Negative_Money_Flow > 1.55711e+08 )
						ret := -0.720930 // sell
		if( Positive_Money_Flow_Sum > 1.08339e+09 )
			if( Positive_Money_Flow_Sum <= 1.49047e+09 )
				if( Positive_Money_Flow <= 8.46905e+07 )
					if( rsi1 <= 72.7127 )
						ret := 0.593548
					if( rsi1 > 72.7127 )
						ret := 0.081081
...
float op_operation = decision_tree_0_FFIV_1Day_962c5b1e(Positive_Money_Flow_Sum, Raw_Money_Flow, MFI_Low, Negative_Money_Flow, Negative_Money_Flow_Sum, Positive_Money_Flow, Money_Flow_Ratio, MFI, MFI_High, Typical_Price, d, d_k, smoothK_k, k, rsi1, smoothD_d)
