			...
			if( lowerKC <= 22.6729 )
				...
				if( rangema > 1.8706 )
					...
					if( upperBB > 26.908 )
						ret := -0.170732
			if( lowerKC > 22.6729 )
				if( avgHighLow <= 35.7067 )
					if( rangema <= 2.32798 )
						ret := -0.714286 // sell
					if( rangema > 2.32798 )
						ret := -0.190476
				if( avgHighLow > 35.7067 )
					if( avgHighLow <= 44.0087 )
						ret := 0.827586 // buy
					if( avgHighLow > 44.0087 )
						ret := -0.380952
		if( dev > 4.74944 )
...
float op_operation = decision_tree_0_RUN_1Day_d8650093(avgHighLow, priceRange, upperBB, upperKC, sqzOffInt, dev, ma, sqzOnInt, lowerBB, rangema, basis, lowerKC, val, noSqzInt)
