			...
			if( val <= -0.006875 )
				...
				if( upperKC > 10.3093 )
					...
					if( val > -0.008175 )
						ret := 0.250000
			if( val > -0.006875 )
				if( priceRange <= 0.031181 )
					if( lowerKC <= 10.0878 )
						ret := 0.800000 // buy
					if( lowerKC > 10.0878 )
						if( avgHighLow <= 10.2587 )
							ret := -0.032258
						if( avgHighLow > 10.2587 )
							ret := -1.000000 // sell
				if( priceRange > 0.031181 )
...
float op_operation = decision_tree_0_SNAP_30Min_de31f349(avgHighLow, priceRange, upperBB, upperKC, sqzOffInt, dev, ma, sqzOnInt, lowerBB, rangema, basis, lowerKC, val, noSqzInt)
