			...
			if( bullPower <= 0.767345 )
				...
				if( ad_mf > 510325 )
					...
					if( ad_mf > 3.52222e+06 )
						ret := -0.666667
			if( bullPower > 0.767345 )
				if( bearPower <= -3.09803 )
					if( bearPower <= -3.58487 )
						ret := -0.200000
					if( bearPower > -3.58487 )
						ret := -1.000000 // sell
				if( bearPower > -3.09803 )
					if( ad_mf <= 1.14472e+06 )
						ret := 0.060914
					if( ad_mf > 1.14472e+06 )
						ret := -0.184332
		if( bbm > 6.96 )
			if( ad_mf <= -2.33004e+06 )
				if( mf <= 0.093829 )
					if( bbm <= 18.165 )
						ret := 0.210526
					if( bbm > 18.165 )
...
float op_operation = decision_tree_0_ADBE_1Day_750208c2(bullPower, bbp, bearPower, bbm, BBPower_Color, ad, mf, ad_mf)
