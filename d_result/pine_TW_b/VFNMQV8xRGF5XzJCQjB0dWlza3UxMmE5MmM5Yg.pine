...
decision_tree_0_TSLA_1Day_12a92c9b(Upper_Band, basis, Lower_Band, bbp, bullPower, BBPower_Color, bearPower, bbm)=>
	...
	if( Upper_Band > 195.681 )
		...
		if( bearPower > 10.0523 )
			if( basis <= 940.329 )
				if( Lower_Band <= 309.372 )
					if( basis <= 309.188 )
						ret := 0.345070
					if( basis > 309.188 )
						ret := -0.586957
				if( Lower_Band > 309.372 )
					if( Upper_Band <= 913.461 )
						ret := 0.706840 // buy
					if( Upper_Band > 913.461 )
						ret := 0.121622
			if( basis > 940.329 )
				if( Upper_Band <= 1165.5 )
					ret := 0.777778 // buy
				if( Upper_Band > 1165.5 )
					if( bbp <= 301.74 )
						ret := -1.000000 // sell
...
float op_operation = decision_tree_0_TSLA_1Day_12a92c9b(Upper_Band, basis, Lower_Band, bbp, bullPower, BBPower_Color, bearPower, bbm)
