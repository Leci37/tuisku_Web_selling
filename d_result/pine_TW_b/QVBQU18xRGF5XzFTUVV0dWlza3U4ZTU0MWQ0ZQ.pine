...
decision_tree_0_APPS_1Day_8e541d4e(val, sqzOnInt, lowerBB, lowerKC, upperKC, dev, rangema, sqzOffInt, upperBB, priceRange, noSqzInt, ma, basis, avgHighLow)=>
	...
	if( dev > 0.14796 )
		...
		if( lowerKC > 65.7482 )
			if( rangema <= 4.08699 )
				if( avgHighLow <= 76.8489 )
					ret := -0.600000
				if( avgHighLow > 76.8489 )
					ret := 0.000000
			if( rangema > 4.08699 )
				if( priceRange <= 10.7424 )
					if( avgHighLow <= 76.745 )
						ret := -0.687500
					if( avgHighLow > 76.745 )
						ret := -0.962963 // sell
				if( priceRange > 10.7424 )
...
float op_operation = decision_tree_0_APPS_1Day_8e541d4e(val, sqzOnInt, lowerBB, lowerKC, upperKC, dev, rangema, sqzOffInt, upperBB, priceRange, noSqzInt, ma, basis, avgHighLow)
