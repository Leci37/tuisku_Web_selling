			...
			if( val > -0.213469 )
				...
				if( rangema > 2.14019 )
					...
					if( dev > 7.79701 )
						ret := -0.888889 // sell
		if( avgHighLow > 43.8526 )
			if( avgHighLow <= 45.4098 )
				if( dev <= 0.514938 )
					ret := 0.846154 // buy
				if( dev > 0.514938 )
					if( priceRange <= 0.50285 )
						ret := 0.227273
					if( priceRange > 0.50285 )
						if( lowerKC <= 43.3861 )
							ret := -0.800000 // sell
...
float op_operation = decision_tree_0_DOTUSDT_1Hour_5815f97d(sqzOnInt, priceRange, lowerKC, lowerBB, rangema, noSqzInt, basis, val, ma, dev, sqzOffInt, upperBB, upperKC, avgHighLow)
