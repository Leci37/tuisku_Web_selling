			...
			if( pvimax <= 2.09382 )
				...
				if( azul > -50.7203 )
					...
					if( BollOsc > 39.0045 )
						ret := -0.676190
			if( pvimax > 2.09382 )
				if( marron_mean <= 92.7371 )
					if( verde_mean <= 71.4124 )
						ret := -0.100000
					if( verde_mean > 71.4124 )
						ret := 0.871795 // buy
				if( marron_mean > 92.7371 )
					if( verde_media <= 36.0166 )
						ret := 0.000000
					if( verde_media > 36.0166 )
						ret := -0.800000 // sell
...
float op_operation = decision_tree_0_LTCUSDT_1Day_be84bd01(verde_mean, verde, marron, media_azul, verde_media, nvi_ema, stoc, azul, verde_azul, pvi_ema, tprice, pvi, marron_mean, pvimax, azul_mean, xrsi, nvimin, media, source, oscp, nvim, xmf, pvimin, BollOsc, nvi, pvim, nvimax)
