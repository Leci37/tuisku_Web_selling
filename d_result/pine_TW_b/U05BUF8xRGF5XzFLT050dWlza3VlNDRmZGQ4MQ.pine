			...
			if( marron > 107.286 )
				...
				if( verde_azul > 124.365 )
					...
					if( BollOsc > 44.4953 )
						ret := 0.214286
	if( pvi > 1.62805 )
		if( azul_mean <= 35.5662 )
			if( tprice <= 12.4626 )
				if( media_azul <= 47.5508 )
					if( source <= 9.84904 )
						ret := 0.214286
					if( source > 9.84904 )
						ret := -0.500000
				if( media_azul > 47.5508 )
					...
					if( xmf > 60.3441 )
						ret := 0.067797
			if( tprice > 12.4626 )
				if( verde_azul <= 233.065 )
					if( azul_mean <= -9.29885 )
						ret := -0.800885 // sell
					if( azul_mean > -9.29885 )
						ret := -0.395760
				if( verde_azul > 233.065 )
...
float op_operation = decision_tree_0_SNAP_1Day_e44fdd81(nvimin, azul, nvim, marron_mean, marron, nvimax, pvi, pvi_ema, oscp, verde, source, stoc, verde_mean, xrsi, media, xmf, tprice, pvimin, media_azul, verde_media, BollOsc, pvimax, nvi_ema, azul_mean, pvim, nvi, verde_azul)
