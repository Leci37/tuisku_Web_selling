				...
				if( bearPower > 0.07174 )
					...
					if( bbm > 0.022431 )
						...
						if( d > 92.4987 )
							ret := -0.181662
		if( bbp > 0.378507 )
			if( bearPower <= 0.270835 )
				if( bullPower <= 0.257806 )
					if( d <= 99.8626 )
						ret := -0.418288
					if( d > 99.8626 )
						ret := 0.724138 // buy
				if( bullPower > 0.257806 )
					ret := -0.062877
			if( bearPower > 0.270835 )
				if( smoothK_k <= 42.0403 )
					ret := -0.837500 // sell
				if( smoothK_k > 42.0403 )
					if( smoothD_d <= 96.8835 )
						if( d_k <= 7.24421 )
							ret := -0.417808
						if( d_k > 7.24421 )
							ret := 0.666667
...
float op_operation = decision_tree_0_TWLO_1Min_a3fe56f3(bbp, bullPower, BBPower_Color, bearPower, bbm, rsi1, d, k, smoothD_d, d_k, smoothK_k)
