			...
			if( ad_mf <= -9.40026e+06 )
				...
				if( mf > -0.044617 )
					...
					if( mf > 0.012277 )
						ret := -0.108108
			if( ad_mf > -9.40026e+06 )
				if( ad_mf <= -8.69713e+06 )
					if( bbp <= 0.797743 )
						ret := -0.714286 // sell
					if( bbp > 0.797743 )
						ret := -1.000000 // sell
				if( ad_mf > -8.69713e+06 )
					if( bullPower <= 35.0433 )
						ret := -0.141434
					if( bullPower > 35.0433 )
						ret := -0.515625
...
float op_operation = decision_tree_0_NFLX_1Day_bf87d7d2(bearPower, bbm, bbp, bullPower, BBPower_Color, mf, ad_mf, ad)
