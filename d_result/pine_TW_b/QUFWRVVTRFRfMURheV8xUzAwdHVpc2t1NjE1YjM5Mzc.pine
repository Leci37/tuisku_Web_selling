			...
			if( smoothK_k > 60.3048 )
				if( rsi1 <= 49.402 )
					...
					if( smoothD_d > 69.431 )
						ret := 0.040541
				if( rsi1 > 49.402 )
					if( smoothK_k <= 64.5426 )
						ret := 0.920000 // buy
					if( smoothK_k > 64.5426 )
						ret := 0.415525
		if( k > 92.9036 )
			if( rsi1 <= 62.2875 )
				if( smoothD_d <= 91.5146 )
					if( smoothK_k <= 92.7472 )
						ret := 0.650000
					if( smoothK_k > 92.7472 )
						ret := -0.034483
				if( smoothD_d > 91.5146 )
...
float op_operation = decision_tree_0_AAVEUSDT_1Day_615b3937(rsi1, smoothK_k, d_k, k, d, smoothD_d)
