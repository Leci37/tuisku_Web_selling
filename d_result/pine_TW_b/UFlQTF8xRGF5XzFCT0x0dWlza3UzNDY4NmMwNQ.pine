			...
			if( BBupper > 231.58 )
				...
				if( dif_source_BBupper > -25.7835 )
					...
					if( vrsi > 75.2035 )
						ret := 0.400000
	if( BBupper > 301.991 )
		if( dif_source_BBupper <= -251.154 )
			if( dif_source_BBlower <= 10.2472 )
				ret := 1.000000 // buy
			if( dif_source_BBlower > 10.2472 )
				if( dif_source_BBlower <= 27.2076 )
					if( BBupper <= 341.555 )
						ret := -0.333333
					if( BBupper > 341.555 )
						ret := -1.000000 // sell
...
float op_operation = decision_tree_0_PYPL_1Day_34686c05(crossunder_rsi_bbupperInt, BBlower, dif_source_BBupper, crossover_rsi_bblowerInt, vrsi, BBupper, dif_source_BBlower)
