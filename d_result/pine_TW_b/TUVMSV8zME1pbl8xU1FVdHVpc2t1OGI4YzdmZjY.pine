	...
	if( rangema > 11.1864 )
		if( lowerBB <= 446.764 )
			...
			if( priceRange > 20.4706 )
				ret := 0.125000
		if( lowerBB > 446.764 )
			if( avgHighLow <= 811.347 )
				if( avgHighLow <= 802.578 )
					if( upperBB <= 831.276 )
						if( dev <= 72.8915 )
							ret := -0.305895
						if( dev > 72.8915 )
...
float op_operation = decision_tree_0_MELI_30Min_8b8c7ff6(sqzOnInt, priceRange, lowerKC, lowerBB, rangema, noSqzInt, basis, val, ma, dev, sqzOffInt, upperBB, upperKC, avgHighLow)
