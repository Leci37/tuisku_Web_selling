		...
		if( smoothD_d > 44.6616 )
			if( mf <= -0.015624 )
				...
				if( smoothD_d > 89.0427 )
					ret := 0.000000
			if( mf > -0.015624 )
				if( ad_mf <= -4.25269e+06 )
					if( smoothK_k <= 75.5765 )
						ret := -1.000000 // sell
					if( smoothK_k > 75.5765 )
						ret := -0.166667
				if( ad_mf > -4.25269e+06 )
...
float op_operation = decision_tree_0_SNOW_1Day_ecb36913(mf, ad, ad_mf, d_k, d, rsi1, k, smoothD_d, smoothK_k)
