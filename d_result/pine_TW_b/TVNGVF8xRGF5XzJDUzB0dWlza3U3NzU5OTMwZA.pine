		...
		if( mf <= -0.021123 )
			...
			if( ad > 5.15354e+07 )
				...
				if( rsi1 > 43.4668 )
					ret := -1.000000 // sell
		if( mf > -0.021123 )
			if( ad_mf <= -1.71676e+07 )
				if( smoothK_k <= 59.6061 )
					if( rsi1 <= 46.4121 )
						ret := -0.592593
					if( rsi1 > 46.4121 )
						ret := -0.970588 // sell
			...
			if( ad_mf > -1.71676e+07 )
				if( mf <= 0.161538 )
					if( ad_mf <= 1.61891e+07 )
						ret := -0.079840
					if( ad_mf > 1.61891e+07 )
						ret := -0.309917
				if( mf > 0.161538 )
					if( ad_mf <= -5.07382e+06 )
						ret := 0.435185
					if( ad_mf > -5.07382e+06 )
						ret := -0.033962
...
float op_operation = decision_tree_0_MSFT_1Day_7759930d(mf, ad_mf, ad, d, d_k, smoothK_k, k, rsi1, smoothD_d)
