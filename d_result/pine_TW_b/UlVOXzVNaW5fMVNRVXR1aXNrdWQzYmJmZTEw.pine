		...
		if( dev > 0.429392 )
			...
			if( lowerKC > 16.8014 )
				if( upperKC <= 17.7768 )
					ret := -0.837696 // sell
				if( upperKC > 17.7768 )
					if( avgHighLow <= 18.1313 )
						if( avgHighLow <= 17.6871 )
							ret := -0.129412
						if( avgHighLow > 17.6871 )
							ret := 0.474359
					if( avgHighLow > 18.1313 )
...
float op_operation = decision_tree_0_RUN_5Min_d3bbfe10(avgHighLow, priceRange, upperBB, upperKC, sqzOffInt, dev, ma, sqzOnInt, lowerBB, rangema, basis, lowerKC, val, noSqzInt)
