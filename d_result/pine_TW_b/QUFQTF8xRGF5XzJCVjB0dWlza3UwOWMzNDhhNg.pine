			...
			if( bullPower <= 1.561 )
				...
				if( bullPower > -0.071746 )
					...
					if( bbm > 6.71 )
						ret := -0.666667
			if( bullPower > 1.561 )
				if( bearPower <= 17.4688 )
					if( bbm <= 11.4109 )
						ret := 0.034342
					if( bbm > 11.4109 )
						ret := 0.846154 // buy
				if( bearPower > 17.4688 )
					if( bearPower <= 26.9023 )
						ret := -1.000000 // sell
					if( bearPower > 26.9023 )
						ret := -0.571429
		if( VIM > 1.41024 )
...
float op_operation = decision_tree_0_AAPL_1Day_09c348a6(bullPower, bbp, bearPower, bbm, BBPower_Color, VIP, VIM, VIP_VIM)
