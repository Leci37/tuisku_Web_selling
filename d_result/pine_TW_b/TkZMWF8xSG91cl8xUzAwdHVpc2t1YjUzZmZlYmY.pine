...
decision_tree_0_NFLX_1Hour_b53ffebf(d, d_k, smoothK_k, k, rsi1, smoothD_d)=>
	...
	if( d_k > 2.39889 )
		...
		if( smoothK_k > 33.3537 )
			...
			if( rsi1 > 72.8491 )
				if( d <= 96.5991 )
					if( smoothD_d <= 58.4667 )
						if( d_k <= 3.16213 )
							ret := 0.285714
						if( d_k > 3.16213 )
							ret := -0.714286 // sell
					if( smoothD_d > 58.4667 )
						if( smoothK_k <= 70.2004 )
							ret := 0.234818
						if( smoothK_k > 70.2004 )
							ret := -0.179310
				if( d > 96.5991 )
...
float op_operation = decision_tree_0_NFLX_1Hour_b53ffebf(d, d_k, smoothK_k, k, rsi1, smoothD_d)
