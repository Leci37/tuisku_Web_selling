			...
			if( verde_mean <= 119.086 )
				if( pvi <= 1.21947 )
					...
					if( tprice > 217.566 )
						ret := -0.059829
				if( pvi > 1.21947 )
					if( media_azul <= 114.943 )
						ret := -0.804598 // sell
					if( media_azul > 114.943 )
						ret := 0.400000
			if( verde_mean > 119.086 )
				if( azul_mean <= 15.0758 )
					if( stoc <= 83.8024 )
						ret := 0.717391 // buy
					if( stoc > 83.8024 )
						ret := 0.000000
				if( azul_mean > 15.0758 )
					if( verde_media <= 47.1677 )
						ret := -0.928571 // sell
					if( verde_media > 47.1677 )
						ret := -0.250000
...
float op_operation = decision_tree_0_SPOT_1Day_3f961f71(nvimin, azul, nvim, marron_mean, marron, nvimax, pvi, pvi_ema, oscp, verde, source, stoc, verde_mean, xrsi, media, xmf, tprice, pvimin, media_azul, verde_media, BollOsc, pvimax, nvi_ema, azul_mean, pvim, nvi, verde_azul)
