			...
			if( avgHighLow <= 56.8716 )
				if( upperKC <= 57.8093 )
					...
					if( dev > 11.8333 )
						ret := 1.000000 // buy
				if( upperKC > 57.8093 )
					if( avgHighLow <= 54.198 )
						ret := 0.966667 // buy
					if( avgHighLow > 54.198 )
						ret := 0.208333
			if( avgHighLow > 56.8716 )
				if( avgHighLow <= 62.3875 )
					if( lowerBB <= 56.8515 )
						ret := -0.349206
					if( lowerBB > 56.8515 )
						ret := -0.698413
...
float op_operation = decision_tree_0_LYFT_1Day_743e95c4(sqzOnInt, priceRange, lowerKC, lowerBB, rangema, noSqzInt, basis, val, ma, dev, sqzOffInt, upperBB, upperKC, avgHighLow)
