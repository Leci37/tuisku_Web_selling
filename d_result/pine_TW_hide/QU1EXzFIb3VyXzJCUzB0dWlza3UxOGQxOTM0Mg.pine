		...
		if( smoothD_d <= -2.4531 )
			if( rsi1 <= 16.619 )
				...
				if( rsi1 > 15.2484 )
					ret := 0.730769 // buy
			if( rsi1 > 16.619 )
				if( bullPower <= -0.840953 )
					if( k <= 1.03197 )
						ret := -0.751825 // sell
					if( k > 1.03197 )
						ret := 0.533333
				if( bullPower > -0.840953 )
					if( smoothD_d <= -2.53309 )
						ret := -0.064257
					if( smoothD_d > -2.53309 )
						ret := -0.622222
		if( smoothD_d > -2.4531 )
...
float op_operation = decision_tree_0_AMD_1Hour_18d19342(bullPower, bbp, bearPower, bbm, BBPower_Color, rsi1, smoothK_k, d_k, k, d, smoothD_d)
