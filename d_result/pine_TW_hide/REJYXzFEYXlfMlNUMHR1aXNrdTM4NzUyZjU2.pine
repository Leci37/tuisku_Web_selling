	...
	if( ema3 > 22.8618 )
		if( d <= 1.33338 )
			if( k <= 0.038114 )
				ret := 0.000000
			if( k > 0.038114 )
				if( smoothK_k <= -1.49754 )
					ret := -1.000000 // sell
				if( smoothK_k > -1.49754 )
					ret := 0.000000
		if( d > 1.33338 )
			if( ema13 <= -0.042146 )
				if( smoothD_d <= 62.3908 )
					if( d <= 1.54942 )
						ret := 0.875000 // buy
					if( d > 1.54942 )
						ret := -0.084337
...
float op_operation = decision_tree_0_DBX_1Day_38752f56(d, d_k, smoothK_k, k, rsi1, smoothD_d, ema12, ema3, ema13, tema, ema1, ema2)
