			...
			if( d <= 70.314 )
				if( k <= 53.3517 )
					...
					if( ad > -194842 )
						ret := 0.016260
				if( k > 53.3517 )
					if( ad_mf <= -1.00937e+06 )
						ret := 0.894737 // buy
					if( ad_mf > -1.00937e+06 )
						ret := 0.142857
			if( d > 70.314 )
				if( smoothK_k <= 59.4358 )
					if( ad <= 1.13862e+06 )
						ret := -0.925000 // sell
					if( ad > 1.13862e+06 )
						ret := 0.500000
...
float op_operation = decision_tree_0_ETSY_1Day_85ff6462(mf, ad_mf, ad, d, d_k, smoothK_k, k, rsi1, smoothD_d)
