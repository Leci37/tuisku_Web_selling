			...
			if( BBupper <= 336.054 )
				...
				if( BBupper > 254.921 )
					...
					if( vrsi > 62.9815 )
						ret := -0.304348
			if( BBupper > 336.054 )
				if( dif_source_BBupper <= -123.576 )
					if( dif_source_BBlower <= 13.4821 )
						ret := 0.956522 // buy
					if( dif_source_BBlower > 13.4821 )
						ret := 0.464286
				if( dif_source_BBupper > -123.576 )
					if( dif_source_BBlower <= 39.8623 )
						ret := -0.783784 // sell
					if( dif_source_BBlower > 39.8623 )
						ret := 0.000000
...
float op_operation = decision_tree_0_SPOT_1Day_0e8d79bf(crossunder_rsi_bbupperInt, BBlower, dif_source_BBlower, dif_source_BBupper, vrsi, BBupper, crossover_rsi_bblowerInt)
