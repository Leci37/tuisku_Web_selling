...
decision_tree_0_SOLUSDT_1Day_277e62b2(bbm, bearPower, bbp, bullPower, BBPower_Color, d_k, d, rsi1, k, smoothD_d, smoothK_k)=>
	...
	if( rsi1 > 73.1397 )
		if( d_k <= 18.2097 )
			if( smoothK_k <= 59.8698 )
				if( d_k <= -10.4901 )
					if( smoothD_d <= 25.2967 )
						ret := 0.000000
					if( smoothD_d > 25.2967 )
						ret := 0.750000 // buy
				if( d_k > -10.4901 )
...
float op_operation = decision_tree_0_SOLUSDT_1Day_277e62b2(bbm, bearPower, bbp, bullPower, BBPower_Color, d_k, d, rsi1, k, smoothD_d, smoothK_k)
