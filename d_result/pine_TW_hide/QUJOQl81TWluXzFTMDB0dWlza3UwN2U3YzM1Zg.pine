				...
				if( rsi1 > 57.2413 )
					...
					if( smoothK_k > 3.05957 )
						...
						if( k > 6.19702 )
							ret := 0.300000
			if( smoothD_d > 1.21657 )
				if( smoothD_d <= 33.4638 )
					if( d_k <= 2.61113 )
						if( smoothK_k <= 52.0188 )
							ret := -0.165398
						if( smoothK_k > 52.0188 )
							ret := -0.582524
					if( d_k > 2.61113 )
						if( smoothD_d <= 17.2874 )
							ret := 0.204301
						if( smoothD_d > 17.2874 )
							ret := -0.124138
				if( smoothD_d > 33.4638 )
...
float op_operation = decision_tree_0_ABNB_5Min_07e7c35f(rsi1, smoothK_k, d_k, k, d, smoothD_d)
