			...
			if( Raw_Money_Flow > 6.74502e+08 )
				...
				if( d > 27.2429 )
					...
					if( MFI > 63.192 )
						ret := 0.294118
	if( MFI > 74.5732 )
		if( Typical_Price <= 5.04923 )
			if( d_k <= -5.57149 )
				ret := -0.923077 // sell
			if( d_k > -5.57149 )
				ret := 0.000000
		if( Typical_Price > 5.04923 )
			if( Negative_Money_Flow_Sum <= 3.02581e+09 )
				if( Typical_Price <= 159.782 )
					if( Negative_Money_Flow_Sum <= 2.85713e+08 )
						ret := 0.355556
					if( Negative_Money_Flow_Sum > 2.85713e+08 )
						ret := 0.809783 // buy
				if( Typical_Price > 159.782 )
					if( smoothK_k <= 95.6879 )
						ret := 0.000000
					if( smoothK_k > 95.6879 )
						ret := -0.571429
			if( Negative_Money_Flow_Sum > 3.02581e+09 )
				if( smoothD_d <= 78.3516 )
					ret := 0.000000
				if( smoothD_d > 78.3516 )
					ret := -0.250000
...
float op_operation = decision_tree_0_SOLUSDT_1Day_010e44a7(Negative_Money_Flow_Sum, MFI_High, Raw_Money_Flow, MFI_Low, Negative_Money_Flow, Money_Flow_Ratio, Typical_Price, MFI, Positive_Money_Flow_Sum, Positive_Money_Flow, d_k, d, rsi1, k, smoothD_d, smoothK_k)
