			...
			if( rsi1 > 25.9683 )
				if( ema12 <= 0.476435 )
					...
					if( k > 48.9363 )
						ret := -0.016241
				if( ema12 > 0.476435 )
					if( smoothK_k <= 32.8583 )
						ret := 0.298611
					if( smoothK_k > 32.8583 )
						ret := -0.038660
		if( ema13 > 26.2829 )
			if( smoothK_k <= 93.5015 )
				if( d <= 94.858 )
					if( d_k <= 0.690952 )
						ret := -1.000000 // sell
					if( d_k > 0.690952 )
...
float op_operation = decision_tree_0_LTCUSDT_1Day_2482829d(d, d_k, smoothK_k, k, rsi1, smoothD_d, ema12, ema3, ema13, tema, ema1, ema2)
