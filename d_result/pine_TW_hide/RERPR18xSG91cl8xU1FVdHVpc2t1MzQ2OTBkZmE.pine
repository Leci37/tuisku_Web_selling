			...
			if( basis <= 104.783 )
				...
				if( lowerBB > 100.599 )
					...
					if( rangema > 1.49314 )
						ret := -0.054945
			if( basis > 104.783 )
				if( avgHighLow <= 105.058 )
					if( priceRange <= 11.665 )
						ret := -0.729885 // sell
					if( priceRange > 11.665 )
						ret := 1.000000 // buy
				if( avgHighLow > 105.058 )
...
float op_operation = decision_tree_0_DDOG_1Hour_34690dfa(sqzOnInt, priceRange, lowerKC, lowerBB, rangema, noSqzInt, basis, val, ma, dev, sqzOffInt, upperBB, upperKC, avgHighLow)
