			...
			if( chinkou_span > 387.85 )
				...
				if( tenkan_sen > 387.975 )
					...
					if( senkou_span_b_displaced > 386.693 )
						ret := 0.175000
	if( tenkan_sen > 389.504 )
		if( chinkou_span <= 389.276 )
			if( basis_max <= 0.058552 )
				if( senkou_span_b_displaced <= 384.835 )
					if( senkou_span_a <= 390.051 )
						ret := -0.923077 // sell
					if( senkou_span_a > 390.051 )
						ret := 0.333333
				if( senkou_span_b_displaced > 384.835 )
					if( chinkou_span <= 385.139 )
						ret := -0.926471 // sell
					if( chinkou_span > 385.139 )
						ret := -0.652174
			if( basis_max > 0.058552 )
				if( kinjun_sen <= 391.687 )
					if( basis_minus <= 0.812982 )
						ret := -0.086957
					if( basis_minus > 0.812982 )
						ret := 0.875000 // buy
				if( kinjun_sen > 391.687 )
					ret := -1.000000 // sell
		if( chinkou_span > 389.276 )
			if( tenkan_sen <= 390.991 )
				if( senkou_span_b_displaced <= 386.078 )
					ret := -0.400000
				if( senkou_span_b_displaced > 386.078 )
					ret := 0.113924
			if( tenkan_sen > 390.991 )
				if( senkou_span_b <= 389.647 )
					ret := 0.190476
				if( senkou_span_b > 389.647 )
					if( senkou_span_a_displaced <= 390.35 )
						if( senkou_span_a_displaced <= 389.863 )
							ret := -0.384615
						if( senkou_span_a_displaced > 389.863 )
							ret := -0.947368 // sell
					if( senkou_span_a_displaced > 390.35 )
...
float op_operation = decision_tree_0_CRWD_5Min_c9d8fc59(senkou_span_a, basis_minus, chinkou_span, senkou_span_a_displaced, kinjun_sen, senkou_span_b_displaced, cross_up_tenkan_kinjunInt, senkou_span_b, cross_dn_tenkan_kinjunInt, tenkan_sen, basis_max)
