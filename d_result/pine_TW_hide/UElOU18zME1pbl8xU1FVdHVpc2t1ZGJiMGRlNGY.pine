	...
	if( lowerBB <= 18.5928 )
		...
		if( val > 0.080854 )
			...
			if( lowerBB > 18.3301 )
				ret := 1.000000 // buy
	if( lowerBB > 18.5928 )
		if( avgHighLow <= 86.2664 )
			if( val <= 0.243768 )
				if( val <= 0.029178 )
					if( avgHighLow <= 62.6559 )
						if( val <= -0.120651 )
							ret := -0.403587
						if( val > -0.120651 )
							ret := -0.006073
...
float op_operation = decision_tree_0_PINS_30Min_dbb0de4f(sqzOnInt, priceRange, lowerKC, lowerBB, rangema, noSqzInt, basis, val, ma, dev, sqzOffInt, upperBB, upperKC, avgHighLow)
