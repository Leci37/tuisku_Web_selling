			...
			if( BBupper <= 302.933 )
				...
				if( vrsi > 53.8858 )
					...
					if( BBlower > 172.299 )
						ret := -0.047619
			if( BBupper > 302.933 )
				if( dif_source_BBupper <= -393.503 )
					if( BBlower <= 383.748 )
						ret := 0.583333
					if( BBlower > 383.748 )
						ret := 0.000000
				if( dif_source_BBupper > -393.503 )
					if( dif_source_BBlower <= 307.885 )
						ret := -0.915493 // sell
					if( dif_source_BBlower > 307.885 )
						ret := -0.242857
...
float op_operation = decision_tree_0_TTD_1Day_8a306a53(dif_source_BBupper, crossover_rsi_bblowerInt, BBupper, vrsi, crossunder_rsi_bbupperInt, dif_source_BBlower, BBlower)
