			...
			if( pvi_ema > 3.40025 )
				...
				if( pvimax > 3.81229 )
					...
					if( nvi > -1.05993 )
						ret := -0.136752
		if( media_azul > 59.3846 )
			if( marron_mean <= 62.397 )
				if( azul_mean <= -33.2361 )
					if( xmf <= 30.9811 )
						ret := -0.500000
					if( xmf > 30.9811 )
						ret := 0.888889 // buy
...
float op_operation = decision_tree_0_NIO_1Day_4b267bbb(verde_mean, verde, marron, media_azul, verde_media, nvi_ema, stoc, azul, verde_azul, pvi_ema, tprice, pvi, marron_mean, pvimax, azul_mean, xrsi, nvimin, media, source, oscp, nvim, xmf, pvimin, BollOsc, nvi, pvim, nvimax)
