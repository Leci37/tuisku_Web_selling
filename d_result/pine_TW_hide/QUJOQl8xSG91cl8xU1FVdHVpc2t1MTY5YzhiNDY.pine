			...
			if( rangema > 2.23751 )
				...
				if( upperBB > 192.675 )
					...
					if( val > 0.935634 )
						ret := -1.000000 // sell
		if( avgHighLow > 195.511 )
			if( priceRange <= 5.90995 )
				if( avgHighLow <= 201.042 )
					if( rangema <= 2.32462 )
						if( avgHighLow <= 200.135 )
							ret := 0.396825
						if( avgHighLow > 200.135 )
							ret := -0.882353 // sell
					if( rangema > 2.32462 )
...
float op_operation = decision_tree_0_ABNB_1Hour_169c8b46(val, sqzOnInt, lowerBB, lowerKC, upperKC, dev, rangema, sqzOffInt, upperBB, priceRange, noSqzInt, ma, basis, avgHighLow)
