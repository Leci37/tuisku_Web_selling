		...
		if( upperBB <= 688.642 )
			if( basis <= 612.317 )
				if( sqzOffInt <= 0.5 )
					ret := -0.750000 // sell
				if( sqzOffInt > 0.5 )
					if( avgHighLow <= 610.702 )
						if( avgHighLow <= 604.564 )
							ret := 0.600000
						if( avgHighLow > 604.564 )
							ret := -0.790541 // sell
					if( avgHighLow > 610.702 )
...
float op_operation = decision_tree_0_HUBS_5Min_edbf9964(sqzOnInt, priceRange, lowerKC, lowerBB, rangema, noSqzInt, basis, val, ma, dev, sqzOffInt, upperBB, upperKC, avgHighLow)
