			...
			if( d_k > 0.975165 )
				...
				if( bearPower > -0.573687 )
					...
					if( rsi1 > 69.3433 )
						ret := 0.437500
		if( smoothD_d > 79.3547 )
			if( bullPower <= 6.99333 )
				if( bearPower <= -1.44515 )
					if( bearPower <= -1.72195 )
						ret := 0.666667
					if( bearPower > -1.72195 )
						ret := 1.000000 // buy
				if( bearPower > -1.44515 )
...
float op_operation = decision_tree_0_DOTUSDT_1Day_012f443d(bearPower, bbm, bbp, bullPower, BBPower_Color, d, d_k, smoothK_k, k, rsi1, smoothD_d)
