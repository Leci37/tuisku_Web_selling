			...
			if( adx <= 51.9692 )
				...
				if( directionalMovementPlus > 0.595 )
					...
					if( diMinus > 33.5278 )
						ret := -0.739130 // sell
			if( adx > 51.9692 )
				if( directionalMovementPlus <= 0.560917 )
					ret := 0.923077 // buy
				if( directionalMovementPlus > 0.560917 )
					ret := 0.200000
		if( dx > 36.8786 )
			if( diPlus <= 43.3966 )
				if( trueRange <= 2.00747 )
					if( adx <= 26.3699 )
						ret := 0.381579
					if( adx > 26.3699 )
						ret := -0.073930
...
float op_operation = decision_tree_0_NIO_1Day_091b4c9e(dx, directionalMovementMinus, diPlus, smoothedDirectionalMovementMinus, trueRange, smoothedDirectionalMovementPlus, smoothedTrueRange, directionalMovementPlus, diMinus, adx)
