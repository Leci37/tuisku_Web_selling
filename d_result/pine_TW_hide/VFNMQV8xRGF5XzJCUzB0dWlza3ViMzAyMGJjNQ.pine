			...
			if( bbm > 56.6238 )
				...
				if( bullPower > 0.225554 )
					...
					if( smoothD_d > 3.07419 )
						ret := -0.527778
	if( bearPower > -8.41365 )
		if( bullPower <= 320.833 )
			if( bullPower <= 24.3248 )
				if( smoothK_k <= 14.6069 )
					if( smoothK_k <= 1.30529 )
						ret := 0.298507
					if( smoothK_k > 1.30529 )
						ret := -0.534722
				if( smoothK_k > 14.6069 )
					if( bullPower <= 13.1048 )
						ret := 0.229773
					if( bullPower > 13.1048 )
						ret := -0.129973
			if( bullPower > 24.3248 )
...
float op_operation = decision_tree_0_TSLA_1Day_b3020bc5(bbp, bullPower, BBPower_Color, bearPower, bbm, rsi1, d, k, smoothD_d, d_k, smoothK_k)
