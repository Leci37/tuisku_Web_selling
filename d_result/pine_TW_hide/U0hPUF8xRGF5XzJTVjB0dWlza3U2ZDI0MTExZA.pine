			...
			if( rsi1 <= 80.7548 )
				if( rsi1 <= 78.0581 )
					...
					if( rsi1 > 57.3603 )
						ret := -0.026534
				if( rsi1 > 78.0581 )
					if( smoothK_k <= 80.4986 )
						ret := -0.041667
					if( smoothK_k > 80.4986 )
						ret := -0.794872 // sell
			if( rsi1 > 80.7548 )
				if( VIP <= 1.34736 )
					if( smoothK_k <= 89.1308 )
						ret := 0.000000
					if( smoothK_k > 89.1308 )
						ret := 0.683333
				if( VIP > 1.34736 )
...
float op_operation = decision_tree_0_SHOP_1Day_6d24111d(d_k, d, rsi1, k, smoothD_d, smoothK_k, VIP_VIM, VIM, VIP)
