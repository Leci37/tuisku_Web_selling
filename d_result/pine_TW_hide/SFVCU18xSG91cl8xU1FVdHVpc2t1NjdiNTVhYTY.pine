	...
	if( upperKC > 127.696 )
		...
		if( rangema > 5.77295 )
			...
			if( dev > 32.3 )
				if( val <= -1.91355 )
					if( rangema <= 13.4941 )
						if( avgHighLow <= 477.65 )
							ret := 0.437500
						if( avgHighLow > 477.65 )
							ret := -0.788732 // sell
					if( rangema > 13.4941 )
						if( priceRange <= 14.9582 )
							ret := 1.000000 // buy
						if( priceRange > 14.9582 )
							ret := -0.090909
				if( val > -1.91355 )
					if( priceRange <= 25.7548 )
						if( rangema <= 20.3008 )
							ret := 0.432432
						if( rangema > 20.3008 )
							ret := -0.928571 // sell
...
float op_operation = decision_tree_0_HUBS_1Hour_67b55aa6(sqzOnInt, priceRange, lowerKC, lowerBB, rangema, noSqzInt, basis, val, ma, dev, sqzOffInt, upperBB, upperKC, avgHighLow)
