			...
			if( oscp > 25.4275 )
				if( stoc <= 91.4158 )
					...
					if( source > 526.358 )
						ret := -0.692308
				if( stoc > 91.4158 )
					if( marron_mean <= 116.797 )
						ret := 0.000000
					if( marron_mean > 116.797 )
						ret := -1.000000 // sell
		if( azul_mean > 6.87528 )
			if( verde_mean <= 160.063 )
				if( oscp <= 19.6442 )
					if( marron <= 89.8337 )
						ret := -0.789474 // sell
					if( marron > 89.8337 )
...
float op_operation = decision_tree_0_NFLX_1Day_deb338c9(verde_mean, verde, marron, media_azul, verde_media, nvi_ema, stoc, azul, verde_azul, pvi_ema, tprice, pvi, marron_mean, pvimax, azul_mean, xrsi, nvimin, media, source, oscp, nvim, xmf, pvimin, BollOsc, nvi, pvim, nvimax)
