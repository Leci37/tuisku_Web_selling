			...
			if( ad_mf > 1.39105e+07 )
				if( mf <= 0.16245 )
					...
					if( k > 12.0131 )
						ret := 0.252688
				if( mf > 0.16245 )
					if( ad_mf <= 4.72197e+07 )
						ret := -0.172414
					if( ad_mf > 4.72197e+07 )
						ret := -0.846154 // sell
		if( d_k > 2.07794 )
			if( ad_mf <= 8.89137e+07 )
				if( ad_mf <= 3.88997e+07 )
					if( d_k <= 13.9832 )
						ret := -0.494737
					if( d_k > 13.9832 )
						ret := -0.791667 // sell
...
float op_operation = decision_tree_0_AMZN_1Day_ecde1ccc(ad, mf, ad_mf, rsi1, smoothK_k, d_k, k, d, smoothD_d)
