			...
			if( bearPower <= 0.671938 )
				...
				if( bearPower > -0.06353 )
					...
					if( bbp > 3.6833 )
						ret := 0.794118 // buy
			if( bearPower > 0.671938 )
				if( bullPower <= 4.06245 )
					if( VIM <= 0.794705 )
						ret := 0.000000
					if( VIM > 0.794705 )
						ret := -0.780303 // sell
				if( bullPower > 4.06245 )
					if( bearPower <= 3.36525 )
						ret := 0.000000
					if( bearPower > 3.36525 )
						ret := -0.735849 // sell
		if( VIP > 1.34877 )
...
float op_operation = decision_tree_0_RUN_1Day_f2084e77(bbm, bearPower, bbp, bullPower, BBPower_Color, VIP_VIM, VIM, VIP)
