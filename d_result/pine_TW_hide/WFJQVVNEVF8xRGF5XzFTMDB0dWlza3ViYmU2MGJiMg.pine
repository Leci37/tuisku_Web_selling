			...
			if( d_k <= -0.144957 )
				...
				if( smoothD_d > 87.6329 )
					...
					if( d > 95.6531 )
						ret := -1.000000 // sell
			if( d_k > -0.144957 )
				if( smoothD_d <= 91.0311 )
					if( k <= 82.9195 )
						ret := 0.000000
					if( k > 82.9195 )
						ret := -1.000000 // sell
				if( smoothD_d > 91.0311 )
					if( smoothK_k <= 95.2409 )
						ret := 0.846154 // buy
					if( smoothK_k > 95.2409 )
						ret := -0.111111
...
float op_operation = decision_tree_0_XRPUSDT_1Day_bbe60bb2(smoothK_k, d_k, k, smoothD_d, rsi1, d)
